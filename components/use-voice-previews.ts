...
export function useVoicePreviews() {
  ...
  const cache = useRef(new Map<VoiceVariantId, string>());
  const pending = useRef<{ variantId: VoiceVariantId; controller: AbortController } | null>(null);
  ...
  async function load(variantId: VoiceVariantId) {
    const variant = getVoiceVariant(variantId);
    if (!variant || pending.current?.variantId === variantId) return;
    pending.current?.controller.abort(); pending.current = null;
    const playId = ++sequence.current;
    const cached = cache.current.get(variantId);
    if (cached) { setPreview({ variantId, playId, loading: false, url: cached }); return; }
    const request = { variantId, controller: new AbortController() };
    pending.current = request;
    setPreview({ variantId, playId, loading: true });
    try {
      // No editor state is passed here: previews always use the profile baseline.
      ...
      cache.current.set(variantId, url);
