      ...
      <form onSubmit={generate} className="workspace">
        <div className="input-column">
          <section className="script-section">
            ...
            <textarea id="script" name="text" value={text} onChange={(e) => setText(e.target.value)} maxLength={MAX_TEXT} required disabled={busy} aria-describedby="script-hint script-count" placeholder="Write the words you want to bring to life…" className="script-input" />
          ...
          <div className="generate-row"><span>{compare ? "Find the voice that fits." : "Make it sound like you mean it."}</span><button className="generate-button" type="submit" disabled={busy || !text.trim() || (compare && comparedVoices.length < 2)}>{busy ? <><span className="spinner" />{compare ? "Generating auditions…" : "Generating voice…"}</> : <><SoundMark small />{compare ? "Generate auditions" : "Generate voice"}</>}</button></div>
